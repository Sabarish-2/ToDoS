...
public class LocalDB extends SQLiteOpenHelper {
    ...
    public ArrayList<TaskModel> fetchTask() {
        ...
        while (cr.moveToNext())
        {
            TaskModel model = new TaskModel(cr.getString(0), cr.getString(1), cr.getInt(2));
            arrTask.add(model);
        }
        cr.close();
        return arrTask;
    ...
    public void editTask(String name, int status, String newName)
    {
        int id = getId(name, status);
        if (id == -1) return;
    ...
    public void delTask(String name, Integer status)
    {
        int id = getId(name, status);
        if (id == -1) return;
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    public int getId(String name, Integer status)
    {
        int id = -1;
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cr = db.rawQuery("SELECT " + TASKS_ID + " FROM " + TABLE_TASKS + " WHERE " + TASKS_NAME + " = ? AND " + TASKS_STATUS + " = ?", new String[]{name, String.valueOf(status)});
        if (cr.moveToFirst())
            id = cr.getInt(0);
        cr.close();
        return id;
    ...
    public TaskModel getTask(String id){
        SQLiteDatabase db = this.getReadableDatabase();
        TaskModel model = new TaskModel("-1", "Default", 0);
                Cursor cr = db.rawQuery("SELECT * FROM " + TABLE_TASKS + " WHERE " + TASKS_ID + " = ?", new String[]{id});
        if (cr.moveToFirst())
            model = new TaskModel(cr.getString(0), cr.getString(1), cr.getInt(2));
        cr.close();
        return model;
    }
    ...
    public int addTask(String name) {
        ...
        Cursor cr = database.rawQuery("SELECT " + TASKS_ID + " FROM " + TABLE_TASKS + " WHERE " + TASKS_NAME + " = ? AND " + TASKS_STATUS + " = ?", new String[]{name, String.valueOf(0)});
